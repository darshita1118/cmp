...
<div class="px-4 py-2">
    <!--begin::Wizard Step 1-->
    <div class="pb-5" >
        <form class="form" id="kt_forms" action="<?= base_url('admin/upload-documents/'.$lid.'/'.$sid) ?>" method="post" enctype="multipart/form-data">
            <div class="row mb-3">
                <?= csrf_field() ?>
                <div class="col-xl-5">
                    <!--begin::Select-->
            ...
            <?php foreach ($student_docs as $student) : ?>
                <div class="col-lg-3 my-3">
                    ...
                    <div class="text-center my-3" style="position: absolute;top: 5px;right: 5px;width: 25px;background: #fff;height: 25px;padding: 5px 5px;border-radius: 50%;box-shadow: 4px 4px 5px 0px #dddcdc, -4px -4px 5px 0px #dddcdc, 4px -4px 5px 0px #dddcdc, -4px 4px 5px 0px #dddcdc;">
                        <a href="<?= base_url('admin/upload-documents/'.$lid.'/'.$sid) ?><?='?delete=' . $student['sd_id'] ?>" style="color:red; font-size: 1rem; font-weight: 500;"><i style="color:red; font-size: 1.3rem;" class="fa fa-trash"></i></a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <form class="form" action="<?= base_url('admin/edit-profile-step-action/'.$lid.'/'.$sid) ?>" method="post">
        <?= csrf_field() ?>
        <!--begin::Wizard Actions-->
        <div class="d-flex justify-content-between border-top mt-5 pt-5">
            <div class="mr-2">
                <a href="<?= base_url('admin/edit-application-form/' . $lid . '/' . $sid . '/academic-detail') ?>" class="btn btn-light-primary font-weight-bolder text-uppercase">Previous</a>
            </div>
            <div>
                <?php
                ...
                <?php else: ?>
                    <button type="button" class="btn btn-primary font-weight-bolder text-uppercase" onclick="showFire('error','Please fill the required documents first.')" >Save & Next</button>
                <?php endif; ?>
            </div>
        </div>
        <!--end::Wizard Actions-->
