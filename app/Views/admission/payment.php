...
if ((old('nature') ?? ($studentInfo['si_course_nature'] ?? '')) == 3 && (($studentInfo['si_stream_group'] ?? '')) == 117) {
    $encryptedAmt = base64_encode($encrypter->encrypt('1500.00'));
} else {
    $encryptedAmt = base64_encode($encrypter->encrypt($feeDetail['amount'] ?? '500.00'));
}
//$encryptedAmt = base64_encode($encrypter->encrypt($feeDetail['amount']??'500.00'));

$deCryptAmt = $encrypter->decrypt(base64_decode($encryptedAmt));
?>
<form class="form" id="kt_forms" action="<?= base_url('payment-initiated/' . $lid . '/' . $sid) ?>" target='_blank' method="post">
    <?= csrf_field() ?>
    ...
    <?php /*
    For test Payment Gateway 
    <input type="hidden" name="urlType" value="<?= 'test-server-JGDbKGJEH%$^%%HGDVBSJKHJKLHS' ?>">
    */ ?>
    <!--1. Application Payment: -->
    <div id="payment" class="form-step active">
        <div class="card border-0 mb-4">
            <div class="card-header bg-none p-3 h3 m-0 ">
                <i class="fa fa-shopping-bag fa-lg me-2 text-gray text-opacity-50"></i>Application Payment:
            ...
            <div class="card-body p-3 text-dark fw-bold border-bottom">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="program" name="program" onchange="getNature($(this).val(),$(this).find(':selected').attr('data-nature'));$('#level').val($(this).find(':selected').attr('data-level')); $('#nature').val($(this).find(':selected').attr('data-nature'));" class="form-select">
                            <option value="">Select</option>
            ...
            <div class="p-2  ms-auto ">
                <button type="submit" class="btn btn-primary font-weight-bolder text-uppercase" value="payment" name="btn">Pay Online</button>
            </div>
            <!--end::Wizard Actions-->
</form>
